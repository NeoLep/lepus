...
- DeepSeek 余额：官方 API 配置可在模型管理中安全查询账户可用、充值与赠送余额。
...
- 定时任务：支持一次、每天和每周计划，在应用运行期间自动执行只读 AI 任务，将结果保存为独立对话并发送桌面通知。
...
远程消息会保存为普通 Lepus 会话，并使用当前选中的模型。你可以为飞书会话分别启用基础工具、联网搜索、本地文件只读访问、自动匹配的 Skills、Skill 脚本、公开网页浏览器操作、内网浏览器访问和剪贴板读取，并限制单条消息的工具轮数。本地文件能力只访问显式选择的工作文件夹；Skill 脚本默认禁用，启用后可以访问网络和本机文件。内网访问仍会阻止 localhost、链路本地、云元数据地址和带 URL 凭据的目标。还可以配置飞书用户 Open ID 白名单；白名单为空时，应用可见范围内的用户都可以发起对话。
