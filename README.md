...
- 自定义模型服务：提供 DeepSeek、OpenAI、Kimi、Qwen 等 Base URL 预设，也可手动配置其他 OpenAI-compatible 服务、模型、密钥与上下文窗口。
- 服务商余额：DeepSeek、Kimi、硅基流动和 OpenRouter 官方 API 配置可在模型管理中安全查询余额或剩余额度。
...
- Skills：创建或导入专业技能，通过 `/` 命令按需启用；从本地文件夹导入的 Skill 会在使用前自动同步源目录更新。
...
远程消息会保存到独立的“飞书（远程对话）”记录入口，并使用当前选中的模型。你可以通过通用能力权限为飞书会话分别启用基础工具、联网搜索、工作区读写、Skills、Skill 脚本、公共或登录态浏览器、剪贴板和下载，并限制单条消息的工具轮数。勾选的能力在远程无人值守场景中视为预授权；本地文件能力仍只访问显式选择的工作文件夹。还可以配置飞书用户 Open ID 白名单；白名单为空时，应用可见范围内的用户都可以发起对话。

每个飞书聊天默认持续使用同一个 Lepus 会话。发送 `/new` 或 `/reset` 可以创建并切换到新的空白会话；也可以使用 `/new 你的问题` 在创建新会话后立即处理第一条消息。旧会话会保留在 Lepus 中。

机器人回复使用飞书富文本消息渲染 Markdown，支持标题、粗体、列表、引用、链接和代码块。超长回复会在保持代码块完整的前提下分段发送；若飞书拒绝某段富文本格式，则自动降级为纯文本。
...
工作流完成后会自动发布一个包含安装包、应用内更新元数据和 `SHA256SUMS.txt` 的 GitHub Release。版本标签与 `package.json` 不一致时，工作流会拒绝构建。
