...
远程消息会保存为普通 Lepus 会话，并使用当前选中的模型。你可以为飞书会话分别启用基础工具、联网搜索、本地文件只读访问、自动匹配的 Skills、公开网页浏览器操作、内网浏览器访问和剪贴板读取，并限制单条消息的工具轮数。本地文件能力只访问显式选择的工作文件夹；写文件、运行脚本和浏览器安装始终禁用。内网访问仍会阻止 localhost、链路本地、云元数据地址和带 URL 凭据的目标。还可以配置飞书用户 Open ID 白名单；白名单为空时，应用可见范围内的用户都可以发起对话。
